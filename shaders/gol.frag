...
void main() {
  ...
  neighbors -= status;
  float t = 1.;
  if (status == 1.0 && (neighbors >= 8. || neighbors <= 1.0)) {
  ...
  vec3 final_color =
      webcamCapture.rgb * (1. - t + 0.8) +
      t * hsv2rgb(color * vec3(sin(lastFrame.r / color.r + time), 0.6, 0.7));
  final_color = mix(final_color * 1.2, lastFrame.rgb, vec3(status));
  gl_FragColor = vec4(mix(final_color, lastFrame, 0.3), 1.);
}
